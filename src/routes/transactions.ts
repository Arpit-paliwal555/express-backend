...
import { Transaction, ITransaction } from "../db";
...
transactionsRouter.get("/book/:bookName", async (req, res) => {
  try {
    ...
    const totalIssuers = new Set(transactions.map((t:ITransaction) => t.userId.toString())).size;

    // Find the current issuer (if any)
    const currentIssue = transactions.find((t:ITransaction) => !t.returnDate);
...
transactionsRouter.get("/rent/:bookName", async (req, res) => {
  try {
    ...
    const totalRent = transactions.reduce((sum:number, transaction: ITransaction) => {
...
transactionsRouter.get("/user-books/:userId", async (req, res) => {
  try {
    ...
    const issuedBooks = transactions.map((transaction: ITransaction) => ({
...
transactionsRouter.get("/issued-books/:startDate/:endDate", async (req, res) => {
  try {
    ...
    const fixDateString = (dateString: string) => dateString.replace(' ', '+');

    const startDateFixed = fixDateString(startDate as string);
    const endDateFixed = fixDateString(endDate as string);
